...
import com.xhub.browser.browser.*
import com.xhub.browser.browser.bookmarks.BookmarksDrawerView
import com.xhub.browser.browser.cleanup.ExitCleanup
import com.xhub.browser.browser.sessions.SessionsPopupWindow
import com.xhub.browser.browser.tabs.TabsDesktopView
import com.xhub.browser.browser.tabs.TabsDrawerView
...
import com.xhub.browser.enums.HeaderInfo
...
import com.xhub.browser.notifications.IncognitoNotification
...
import com.xhub.browser.search.SuggestionsPopup
import com.xhub.browser.settings.NewTabPosition
import com.xhub.browser.settings.fragment.BottomSheetDialogFragment
import com.xhub.browser.fragment.SearchOverlayFragment
import com.xhub.browser.settings.fragment.DisplaySettingsFragment.Companion.MAX_BROWSER_TEXT_SIZE
import com.xhub.browser.settings.fragment.DisplaySettingsFragment.Companion.MIN_BROWSER_TEXT_SIZE
import com.xhub.browser.settings.fragment.SponsorshipSettingsFragment
import com.xhub.browser.ssl.SslState
import com.xhub.browser.ssl.createSslDrawableForState
import com.xhub.browser.ssl.showSslDialog
import com.xhub.browser.utils.*
import com.xhub.browser.view.*
import com.xhub.browser.view.SearchView
...
abstract class WebBrowserActivity : ThemedBrowserActivity(),
    ...
    private var suggestionsPopup: SuggestionsPopup? = null
    ...
    @Inject lateinit var faviconModel: com.xhub.browser.favicon.FaviconModel
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        // Intro removed - skip terms check and go straight to browser
        // Mark terms as accepted so other checks don't fail
...
//        }
    ...
    private fun showSessions() {
        ...
        if (tabsView is TabsDesktopView || tabsDialog.isShowing || !tabBarInDrawer || drawerOpened) {
            ...
            buttonSessions?.let { iMenuSessions.show(it) } ?: iMenuSessions.show(iBindingToolbarContent.tabsButton)
        }
        else {
            // Otherwise use tabs button as anchor since main menu button is removed
            // Certainly calling sessions dialog from main menu option
            iBindingToolbarContent.tabsButton?.let { iMenuSessions.show(it) }
        }
    }
    ...
    private fun doShowMenuMain() {
        ...
        // Open our menu
        iMenuCustom.show(iBindingToolbarContent.buttonMore ?: iBindingToolbarContent.tabsButton)
    }
    ...
    private fun createToolbar() {
        ...
        val toolbarSwipeDetector = GestureDetectorCompat(this, object : GestureDetector.SimpleOnGestureListener() {
            ...
            override fun onFling(event1: MotionEvent?, event2: MotionEvent, velocityX: Float, velocityY: Float): Boolean {
                ...
                // ── VERTICAL swipes: open tabs / search ───────────────────────
                if (dY > kMinSwipeDistance && dX < kMaxSwipeDistance) {
                    if (velocityY < 0) {
                        // Swipe UP → open the Tab Switcher / Tabs panel
                        Timber.d("Toolbar swipe UP → open tabs")
                        openTabs()
                        return true
                    } else {
                        // Swipe DOWN → focus the search bar (open search overlay)
                        Timber.d("Toolbar swipe DOWN → focus search")
    ...
    private fun initialize(savedInstanceState: Bundle?) {
        ...
        // Show incognito icon in more menu button (Removed buttonMore)
        if (isIncognito()) {
            // iBindingToolbarContent.tabsButton.setImageResource(R.drawable.ic_incognito)
        }
        ...
        iBindingToolbarContent.buttonBookmarks?.setOnClickListener { 
            val bottomSheet = com.xhub.browser.fragment.HistoryBookmarksBottomSheet()
            bottomSheet.show(supportFragmentManager, "HistoryBookmarksBottomSheet")
        }
        iBindingToolbarContent.buttonNewTab?.setOnClickListener { executeAction(R.id.action_new_tab) }
        ...
        // Assistant pill interactions
        iBindingToolbarContent.toolbarMicButton?.setOnClickListener {
            ...
            try {
                ...
                snackbar(R.string.title_error)
            }
        }

        iBindingToolbarContent.toolbarAudioButton?.setOnClickListener {
            com.xhub.browser.fragment.AskOverlayFragment().show(supportFragmentManager, "AskOverlayFragment")
        }
        ...
        searchView = iBindingToolbarContent.addressBarInclude.search.apply {
            ...
            // Dynamic logic for the pill's action button
            addTextChangedListener(object : android.text.TextWatcher {
                override fun beforeTextChanged(s: CharSequence?, start: Int, count: Int, after: Int) {}
                override fun onTextChanged(s: CharSequence?, start: Int, before: Int, count: Int) {
                    val hasText = !s.isNullOrBlank()
                    // If you just want the existing forward icon there, maybe change it to audio if NO text.
                    // Assuming id/toolbarAudioButton holds an ImageView at index 0
                    val iv = iBindingToolbarContent.toolbarAudioButton?.getChildAt(0) as? ImageView
                    if (hasText) {
                        iv?.setImageResource(R.drawable.ic_action_forward)
                    } else {
        ...
        // setupButtonMore() // Removed as buttonMore is no longer in layout
    }
    ...
    private fun setupButtonMore() {
        ...
        iBindingToolbarContent.tabsButton.setOnClickListener {
...
//            }
    ...
    */
     ...
     * so we only ever need to show/hide the overlay itself. The web containers MUST remain VISIBLE
     * at all times — hiding them (setting GONE) causes the WebView hardware renderer to discard
     * its backing surface, resulting in a black screen when they become visible again.
     */
    private fun updateHomeScreenOverlay() {
        ...
        val wasShowing = targetHomeScreenState
        ...
        // Only animate the overlay — NEVER hide the web containers
        iBinding.homeScreenOverlay.animate().cancel()

        if (isHome) {
            // Web → Home: fade the overlay IN over the web content
            if (duration > 0L) {
                iBinding.homeScreenOverlay.alpha = 0f
            } else {
            ...
            iTabViewContainerFront.getChildAt(0)?.requestLayout()
            iTabViewContainerFront.getChildAt(0)?.invalidate()
            Timber.d("updateHomeScreenOverlay: forcing invalidate and requestLayout on web containers before fade")
            iBinding.homeScreenOverlay.animate().alpha(0f).setDuration(duration).withEndAction {
                iBinding.homeScreenOverlay.isVisible = false
                iBinding.homeScreenOverlay.alpha = 1f // Reset alpha for next use
                // Force another invalidate after fade completes
                iTabViewContainerFront.invalidate()
                iTabViewContainerFront.getChildAt(0)?.invalidate()
                Timber.d("updateHomeScreenOverlay: fade done, front.vis=${iTabViewContainerFront.visibility} " +
                        "front.childCount=${iTabViewContainerFront.childCount} " +
                        "front.child0=${iTabViewContainerFront.getChildAt(0)?.javaClass?.simpleName}")
            }
        }
    }

    /**
     * Snap the home screen overlay to its correct final visual state immediately (no animation).
     * Called from onResume to repair broken mid-animation states that can occur when the user
     * opens Settings or another Activity during a Home<→Web transition.
     *
     * Web containers are intentionally never touched here — they stay VISIBLE at all times.
     */
    private fun snapHomeScreenOverlayState() {
        ...
        // Always sync targetHomeScreenState so subsequent calls to updateHomeScreenOverlay()
        // correctly detect state changes and trigger the animated transition
        targetHomeScreenState = isHome
    }
    ...
    private fun setupToolBar() {
        // Always show back/forward buttons - they are essential navigation controls
        iBindingToolbarContent.buttonActionBack.isVisible = true
        iBindingToolbarContent.buttonActionForward.isVisible = true
        
        // Check if toolbar is wide enough for tab bar action buttons
        val threshold = (iBindingToolbarContent.buttonActionBack.width?:3840)*10
        if (tabsView is TabsDesktopView) {
            (tabsView as TabsDesktopView).iBinding.actionButtons.isVisible = 
                (iBinding.toolbarInclude.toolbar.width > threshold)
        }
    ...
    override fun executeAction(@IdRes id: Int): Boolean {
        ...
        when (id) {
            ...
            R.id.action_translate -> {
                // Get our local
                val locale = com.xhub.browser.locale.LocaleUtils.requestedLocale(userPreferences.locale)
    ...
    /**
     * From [WebBrowser].
     * Notifies the tab view that a tab at the given position has changed.
     * This should only update the tab list item, not trigger full UI reconfiguration.
     */
    override fun notifyTabViewChanged(position: Int) {
        Timber.d("Notify Tab Changed: $position")
        tabsView?.tabChanged(position)
        // Only update toolbar color, don't reconfigure pull-to-refresh for every metadata change
        setToolbarColor()
    }
     ...
     * Only updates UI elements that are affected by page load start.
     */
    override fun onPageStarted(aTab: WebPageTab) {
        if (tabsManager.currentTab==aTab) {
            setTaskDescription()
            // NOTE: Do NOT call updateHomeScreenOverlay() here.
            // The WebView has not rendered any content yet at this point (onPageStarted fires
            // before the first frame is drawn), so hiding the home overlay would reveal a
            // blank/black WebView. The transition is handled in onTabChanged which is called
            // from onPageFinished once the page actually has content to show.
            
            // Update pull-to-refresh state since page load affects scrollability
            setupPullToRefresh(resources.configuration)
        }

        // Update tab list item for this tab
        doTabUpdate(aTab)
    }

    /**
     * Called when the tab URL changes.
     * Only updates URL-related UI elements.
     */
    override fun onTabChangedUrl(aTab: WebPageTab) {
        ...
        if (tabsManager.currentTab==aTab) {
            ...
            updateToolBarText()
            // Only show the home overlay immediately when navigating TO home (Web→Home).
            // Do NOT hide the home overlay for Home→Web here — the WebView has no content yet
    ...
    /**
     * Called when tab content or state changes significantly.
     * Updates UI elements that depend on tab content being ready.
     */
    override fun onTabChanged(aTab: WebPageTab) {
        ...
        if (tabsManager.currentTab==aTab) {
            setTaskDescription()
            updateHomeScreenOverlay()
            // Update pull-to-refresh state since content change may affect scrollability
            setupPullToRefresh(resources.configuration)
        }
    ...
    /**
     * Called when the tab icon/favicon changes.
     * Only updates icon-related UI elements.
     */
    override fun onTabChangedIcon(aTab: WebPageTab) {
        ...
        // Update only the tab list item for this tab
        doTabUpdate(aTab)
    }

    /**
     * Called when the tab title changes.
     * Only updates title-related UI elements.
     */
    ...
    /**
     * Update tab view for the given tab.
     * Guards against invalid tab indices from stale callbacks.
     */
    private fun doTabUpdate(aTab: WebPageTab) {
        val index = tabsManager.indexOfTab(aTab)
        if (index >= 0) {
            notifyTabViewChanged(index)
        } else {
            Timber.w("doTabUpdate: Tab not found in manager, skipping update (tab id=${aTab.id})")
        }
    }
...
//                }
    ...
    private fun animateTabInScaleUp(aTab: View?) {
        // Runtime check: ensure no animation is in progress
        if (iTabAnimator != null) {
            Timber.e("animateTabInScaleUp: Unexpected animator state - iTabAnimator should be null")
        }
        aTab?.let{
    ...
    private fun animateTabOutScaleDown(aTab: View?) {
        // Runtime check: ensure no animation is in progress
        if (iTabAnimator != null) {
            Timber.e("animateTabOutScaleDown: Unexpected animator state - iTabAnimator should be null")
        }
        aTab?.let{
            iTabAnimator = it.animate()
                    .scaleY(0f)
    ...
    private fun animateTabOutRight(aTab: View?) {
        // Runtime check: ensure no animation is in progress
        if (iTabAnimator != null) {
            Timber.e("animateTabOutRight: Unexpected animator state - iTabAnimator should be null")
        }
        aTab?.let{
    ...
    private fun animateTabOutLeft(aTab: View?) {
        // Runtime check: ensure no animation is in progress
        if (iTabAnimator != null) {
            Timber.e("animateTabOutLeft: Unexpected animator state - iTabAnimator should be null")
        }
        aTab?.let{
            // Move our tab to a frame were we can animate it on top of our new foreground tab
            iTabAnimator = it.animate()
    ...
    private fun animateTabFlipLeft(aTab: View?) {
        // Runtime check: ensure no animation is in progress
        if (iTabAnimator != null) {
            Timber.e("animateTabFlipLeft: Unexpected animator state - iTabAnimator should be null")
        }
        aTab?.let{
    ...
    private fun animateTabFlipRight(aTab: View?) {
        // Runtime check: ensure no animation is in progress
        if (iTabAnimator != null) {
            Timber.e("animateTabFlipRight: Unexpected animator state - iTabAnimator should be null")
        }
    ...
    private fun initializeToolbarHeight(configuration: Configuration) =
            iBinding.uiLayout.doOnLayout {
                iBinding.toolbarInclude.toolbar.layoutParams.height = ViewGroup.LayoutParams.WRAP_CONTENT
                iBinding.toolbarInclude.toolbar.minimumHeight = 0
    ...
    private fun applyToolbarColor(color: Int) {
        ...
        // Icons for bottom toolbar should remain consistent with the Comet design
        iBindingToolbarContent.homeButton.setColorFilter(Color.WHITE)
        iBindingToolbarContent.buttonActionBack.setColorFilter(Color.WHITE)
        iBindingToolbarContent.buttonActionForward.setColorFilter(Color.WHITE)
        ...
        iBindingToolbarContent.addressBarInclude.searchSslStatus.setColorFilter(Color.WHITE)
        // Toolbar buttons filter
        iBindingToolbarContent.buttonReload.setColorFilter(Color.WHITE)


        // Pull to refresh spinner premium makeover
        // Use a dark, premium background for the spinner circle itself
        val spinnerBgColor = ContextCompat.getColor(this, R.color.home_secondary)
        // Use our primary cyan brand color for the loading arrow
        val spinnerArrowColor = ContextCompat.getColor(this, R.color.home_primary)
        
        iTabViewContainerFront.setProgressBackgroundColorSchemeColor(spinnerBgColor)
        iTabViewContainerFront.setColorSchemeColors(spinnerArrowColor)
        iTabViewContainerBack.setProgressBackgroundColorSchemeColor(spinnerBgColor)
        iTabViewContainerBack.setColorSchemeColors(spinnerArrowColor)

        // Make the pull distance shorter and snappier (premium feel)
        val density = resources.displayMetrics.density
        val startOffset = (24 * density).toInt()
        val endOffset = (64 * density).toInt()
        iTabViewContainerFront.setProgressViewOffset(false, startOffset, endOffset)
        iTabViewContainerBack.setProgressViewOffset(false, startOffset, endOffset)

        // Color also applies to the following backgrounds as they show during tool bar show/hide animation
        iBinding.uiLayout.setBackgroundColor(effectiveColor)
        iTabViewContainerFront.setBackgroundColor(effectiveColor)
        ...
        iTabViewContainerBack.setBackgroundColor(effectiveColor)
        ...
        currentTabView?.let {
            ...
            if (iBinding.toolbarInclude.progressView.progress >= 100
                ...
                mainHandler.removeCallbacks(resetBackgroundColorRunnable)
                it.setBackgroundColor(effectiveColor)
                // Make sure that color is applied on the spot for earlier color change when loading tabs
                it.invalidate()
            }
        }

        // Keep toolbar background dark and consistent - Comet design
        iBinding.toolbarInclude.toolbarLayout.setBackgroundColor(effectiveColor)
        iBinding.toolbarInclude.progressView.mProgressColor = getColor(R.color.comet_teal)
    ...
    private fun setSearchBarColors(aColor: Int) {
        if (!::iBindingToolbarContent.isInitialized) return
        val background = iBindingToolbarContent.addressBarInclude.root.background
        (background as? StateListDrawable)?.apply {
            // Order and states may matter depending of what is declared in our background drawable, e.g. [R.drawable.card_bg_elevate]
            drawableForState(android.R.attr.state_focused).tint(ThemeUtils.getSearchBarFocusedColor(aColor))
            drawableForState(android.R.attr.state_enabled).tint(ThemeUtils.getSearchBarColor(aColor))
        }
    }
    ...
    override fun onActivityResult(requestCode: Int, resultCode: Int, intent: Intent?) {
        if (requestCode == 10101 && resultCode == Activity.RESULT_OK) {
            val matches = intent?.getStringArrayListExtra(android.speech.RecognizerIntent.EXTRA_RESULTS)
            val topResult = matches?.firstOrNull()
            if (!topResult.isNullOrEmpty()) {
                searchTheWeb(topResult)
            }
    ...
    private fun toggleToolBar() : Boolean
    {
        return if (isToolBarVisible()) {
            ...
            showActionBar()
            iBindingToolbarContent.tabsButton.requestFocus()
            true
        }
    }
    ...
    /**
     * Handle home screen shortcut clicks.
     */
    ...
    override fun onClick(v: View) {
        ...
        when (v.id) {
            R.id.homeButton -> currentTab.apply { requestFocus(); loadHomePage() }
            // When not using drawer for tabs that button is used to show webpage menu
            R.id.tabsButton -> if (configPrefs.tabBarInDrawer) openTabs() else {showMenuWebPage()}
            R.id.buttonReload -> refreshOrStop()
            R.id.button_next -> currentTab.findPrevious()
    ...
    private fun checkForUpdates() {
        ...
        Timber.d("checkForUpdates - DISABLED (no XHub update server)")
    }
    ...
    lateinit var iShortcuts: com.xhub.browser.keyboard.Shortcuts
    ...
    private fun createKeyboardShortcuts() {
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.N) {
            iShortcuts = com.xhub.browser.keyboard.Shortcuts(this)
        }
    }
