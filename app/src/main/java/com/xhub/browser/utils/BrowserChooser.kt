    ...
    private fun showBrowserChooserBottomSheet(context: Context, aUrl: String, excludeThisApp: Boolean, extras: android.os.Bundle?, onDismiss: (() -> Unit)?) {
        try {
            ...
            val resolveInfos = allResolveInfos.filter { resolveInfo ->
                ...
                if (isOurApp && activityName == "com.xhub.browser.activity.SelectBrowserActivity") {
                    return@filter false
                }
            ...
            if (!excludeThisApp) {
                val mainActivityName = "com.xhub.browser.activity.MainActivity"
                val incognitoActivityName = "com.xhub.browser.activity.IncognitoActivity"
