...
class FaviconModel @Inject constructor(
    ...
    fun faviconForUrl(url: String, title: String, aOnDark: Boolean): Maybe<Bitmap> = Maybe.create {
        ...
        val downloaded = downloadFaviconForHost(uri.host)
        if (downloaded != null) {
     ...
     */
    fun realFaviconForUrl(url: String, aOnDark: Boolean): Maybe<Bitmap> = Maybe.create {
    ...
    /**
     * Downloads the best available favicon for [host] by trying several public sources in order:
     * 1. `https://{host}/favicon.ico`
     * 2. DuckDuckGo icon service: `https://icons.duckduckgo.com/ip3/{host}.ico`
     * 3. Google S2 service: `https://www.google.com/s2/favicons?domain={host}&sz=128`
     *
