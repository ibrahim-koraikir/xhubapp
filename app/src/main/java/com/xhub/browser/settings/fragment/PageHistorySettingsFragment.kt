...
class PageHistorySettingsFragment : AbstractSettingsFragment() {
            ...
            history.getItemAtIndex(i).let {item ->
                ...
                if (pref.icon==null) {
                    faviconModel.faviconForUrl(item.url,"",context?.isDarkTheme() == true)
                        ...
                        .subscribeBy(
                            onSuccess = { bitmap ->
                                pref.icon = bitmap.scale(com.xhub.browser.utils.Utils.dpToPx(24f), com.xhub.browser.utils.Utils.dpToPx(24f)).toDrawable(resources)
                            }
                        )
                }
    ...
    override fun onCreateOptionsMenu(menu: android.view.Menu, inflater: android.view.MenuInflater) {
        ...
        val clearItem = menu.add(0, R.id.action_clear, 0, R.string.action_clear)
        clearItem.setIcon(R.drawable.ic_action_delete)
        clearItem.setShowAsAction(android.view.MenuItem.SHOW_AS_ACTION_IF_ROOM)
    }

    override fun onOptionsItemSelected(item: android.view.MenuItem): Boolean {
        if (item.itemId == R.id.action_clear) {
            tabsManager.currentTab?.webView?.clearHistory()
            history = tabsManager.currentTab?.webView?.copyBackForwardList()!!
            currentIndex = history.currentIndex
            populateHistory()
            return true
