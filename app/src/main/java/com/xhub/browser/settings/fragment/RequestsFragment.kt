...
import com.xhub.browser.view.WebPageClient
...
class RequestsFragment : AbstractSettingsFragment() {
    ...
    private var sortMode = SortMode.DOMAIN
    ...
    override fun onCreateOptionsMenu(menu: android.view.Menu, inflater: android.view.MenuInflater) {
        ...
        val sortItem = menu.add(0, R.id.action_sort, 0, R.string.action_sort)
        sortItem.setIcon(R.drawable.ic_sort)
        sortItem.setShowAsAction(android.view.MenuItem.SHOW_AS_ACTION_IF_ROOM)
    }

    override fun onOptionsItemSelected(item: android.view.MenuItem): Boolean {
        if (item.itemId == R.id.action_sort) {
            sortMode = if (sortMode == SortMode.DOMAIN) SortMode.TIME else SortMode.DOMAIN
            populateRequestsList()
            return true
        }
        ...
        if (sortMode == SortMode.DOMAIN) {
            ...
            requestsByDomain.keys.sorted().forEach { domain ->
                ...
                domainRequests.forEach { request ->
                    category.addPreference(createRequestPreference(request))
                }
            }
        } else {
            // Sort by time (latest first)
            val category = PreferenceCategory(requireContext()).apply {
                title = getString(R.string.page_requests_by_time)
                isIconSpaceReserved = false
            }
            preferenceScreen.addPreference(category)
    ...
    private fun createRequestPreference(request: WebPageClient.PageRequest): x.Preference {
