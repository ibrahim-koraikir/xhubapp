package com.xhub.browser.dialog
...
import com.xhub.browser.database.asFolder
...
import com.xhub.browser.extensions.onConfigurationChange
import com.xhub.browser.extensions.onFocusGained
...
import com.xhub.browser.utils.shareUrl
...
class LightningDialogBuilder @Inject constructor(
    ...
    private val downloadHandler: com.xhub.browser.download.DownloadHandler,
    ...
    fun showLongPressLinkImageDialog(
        ...
        BrowserDialog.show(
            activity, 0, "", false,
            //Link tab
            DialogTab(show = showLinkTab, icon = R.drawable.ic_link, title = R.string.button_link, items = arrayOf(DialogItem(title = R.string.dialog_open_new_tab, text = linkUrl) {
                webBrowser.handleNewTab(NewTab.FOREGROUND, linkUrl)
            },
                DialogItem(title = R.string.dialog_open_background_tab, text = linkUrl) {
                ...
                DialogItem(title = R.string.action_share, text = linkUrl) {
                    activity.shareUrl(linkUrl, null)
                },
                // Show copy text dialog item if we have some text
                DialogItem(title = R.string.dialog_copy_text, text = text, show = !text.isNullOrEmpty()) {
            ...
            DialogTab(show = showImageTab, icon = R.drawable.ic_image, title = R.string.button_image,
                items = arrayOf(DialogItem(title = R.string.dialog_open_new_tab, text = imageUrl) {
                    webBrowser.handleNewTab(NewTab.FOREGROUND, imageUrl)
                },
                    DialogItem(title = R.string.dialog_open_background_tab, text = imageUrl) {
                    ...
                    DialogItem(title = R.string.action_share, text = imageUrl) {
