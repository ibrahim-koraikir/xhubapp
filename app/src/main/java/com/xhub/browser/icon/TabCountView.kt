...
import com.xhub.browser.extensions.scale
...
class TabCountView @JvmOverloads constructor(
    ...
    private fun updateBitmap()
    {
        ...
        if (bitmap==null && width > 0 && height > 0) {
            ...
            // Draw a square based on the smaller dimension, centered in the view
            val dimension = Math.min(width, height).toFloat()
            val left = (width - dimension) / 2f
            val top = (height - dimension) / 2f
            workingRect.set(left, top, left + dimension, top + dimension)
    ...
    override fun onDraw(canvas: Canvas) {
        ...
        // Draw a stack of two squares for the Comet design
        paint.style = Paint.Style.STROKE
        paint.strokeWidth = 1.5f * context.resources.displayMetrics.density
        
        // Offset for the back square (5dp offset)
        val offset = 5f * context.resources.displayMetrics.density
        
        // Save state 
        val originalLeft = workingRect.left
        val originalTop = workingRect.top
        
        // Draw the back square (offset right and up)
        workingRect.offset(offset / 1.5f, -offset / 1.5f)
        bitmapCanvas?.drawRoundRect(workingRect, borderRadius, borderRadius, paint)
        
        // Draw the front square (Back to original position)
        workingRect.offset(-offset / 1.5f, offset / 1.5f)
        bitmapCanvas?.drawRoundRect(workingRect, borderRadius, borderRadius, paint)
