...
import com.xhub.browser.adblock.AdBlocker
import com.xhub.browser.adblock.NoOpAdBlocker
import com.xhub.browser.browser.WebBrowser
import com.xhub.browser.activity.FRAGMENT_CLASS_NAME
import com.xhub.browser.activity.SettingsActivity
import com.xhub.browser.activity.WebBrowserActivity
import com.xhub.browser.settings.fragment.PREFERENCE_KEY
import com.xhub.browser.di.HiltEntryPoint
import com.xhub.browser.di.configPrefs
import com.xhub.browser.extensions.getDrawable
import com.xhub.browser.extensions.getText
...
import com.xhub.browser.settings.NoYesAsk
...
import com.xhub.browser.userscript.UserScript
...
import com.xhub.browser.enums.LogLevel
...
class WebPageClient(
    ...
    val networkEngineManager: com.xhub.browser.network.NetworkEngineManager = hiltEntryPoint.networkEngineManager
    val userScriptManager: com.xhub.browser.userscript.UserScriptManager = hiltEntryPoint.userScriptManager
...
/*        // TODO: element hiding does not work
        ...
        if (userPreferences.extensionsEnabled) {
            val scriptCode = userScriptManager.getInjectionCode(url, com.xhub.browser.userscript.RunAt.DOCUMENT_END)
            if (scriptCode != null) {
            ...
            val idleScriptCode = userScriptManager.getInjectionCode(url, com.xhub.browser.userscript.RunAt.DOCUMENT_IDLE)
    ...
    override fun onPageStarted(view: WebView, url: String, favicon: Bitmap?) {
        ...
        if (userPreferences.extensionsEnabled) {
            val scriptCode = userScriptManager.getInjectionCode(url, com.xhub.browser.userscript.RunAt.DOCUMENT_START)
            if (scriptCode != null) {
    ...
    override fun onReceivedError(webview: WebView, errorCode: Int, error: String, failingUrl: String) {
        ...
        // Use non-blocking view posting to apply theming asynchronously
        // This ensures the error page DOM is ready without blocking the UI thread
        webview.post {
    ...
    private fun handleUserScriptInstallation(url: String) {
        ...
        userScriptScope.coroutineLaunch {
            ...
            try {
                // Safety constants
                val MAX_SCRIPT_SIZE = 5 * 1024 * 1024 // 5MB max script size
                val CONNECT_TIMEOUT = 15000 // 15 seconds
                val READ_TIMEOUT = 30000 // 30 seconds
                ...
                if (connection.responseCode == java.net.HttpURLConnection.HTTP_OK) {
                    ...
                    if (contentLength > MAX_SCRIPT_SIZE) {
                        withContext(Dispatchers.Main) {
                            activity.makeSnackbar(
                                activity.getString(R.string.error_userscript_too_large),
                                KDuration,
                                Gravity.BOTTOM
                            ).show()
                        }
                    ...
                    // Stream-read with hard cap to prevent memory exhaustion
                    ...
                    try {
                        ...
                        while (reader.read(charBuffer).also { charsRead = it } != -1) {
                            totalBytesRead += charsRead * 2 // Approximate byte count (UTF-16)
                            
                            // Enforce hard size limit during streaming
                            if (totalBytesRead > MAX_SCRIPT_SIZE) {
                                withContext(Dispatchers.Main) {
                                    activity.makeSnackbar(
                                        activity.getString(R.string.error_userscript_too_large),
                                        KDuration,
                    ...
                    withContext(Dispatchers.Main) {
                        activity.makeSnackbar(
                            activity.getString(R.string.error_downloading_userscript),
                            KDuration,
                            Gravity.BOTTOM
                        ).show()
                    }
                ...
                Timber.e(e, "Timeout downloading userscript")
                withContext(Dispatchers.Main) {
                    activity.makeSnackbar(
                        activity.getString(R.string.error_userscript_timeout),
                        KDuration,
                        Gravity.BOTTOM
                    ).show()
                }
                ...
                // Guaranteed cleanup - always disconnect
                try {
                    connection?.disconnect()
                } catch (e: Exception) {
                    Timber.w(e, "Error disconnecting userscript download connection")
                }
            }
        }
    }
        ...
        if (scriptId != null) {
            ...
            snackbar.setAction(R.string.settings) {
                ...
                val intent = Intent(activity, SettingsActivity::class.java).apply {
                    putExtra(FRAGMENT_CLASS_NAME, "com.xhub.browser.settings.fragment.ExtensionsSettingsFragment")
