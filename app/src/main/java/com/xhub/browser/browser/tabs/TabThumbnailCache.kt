...
object TabThumbnailCache {
    ...
    private fun diskDir(): File? = customCacheDir ?: try {
        File(com.xhub.browser.app.cacheDir, "tab_thumbnails")
    } catch (_: Exception) {
        null
    }
    ...
    fun get(tabId: Int, persistable: Boolean = true, onLoaded: ((Bitmap?) -> Unit)? = null): Bitmap? {
        ...
        // Atomically decide whether to start a new disk load or just enqueue this callback.
        var startDiskLoad = false
        synchronized(pendingCallbacks) {
            val existingList = pendingCallbacks[tabId]
            if (existingList != null) {
                // A disk load is already in flight — just append the callback.
                if (onLoaded != null) {
                ...
                // No in-flight load — register this callback and flag that we should start one.
        ...
        val file = diskDir()?.let { File(it, "$tabId.jpg") } ?: run {
            ...
            list?.let { synchronized(it) { for (cb in it) cb(null) } }
            return null
        }
            ...
            try {
                ...
                mainHandler.post {
                    val finalBitmap = if (bitmap != null) {
                        // Re-check cache before putting: an in-memory bitmap may have been stored
                        // while the disk load was in flight. If so, prefer the in-memory version.
                        val existing = cache.get(tabId)
                        if (existing != null && !existing.isRecycled) {
                            // In-memory bitmap already present — discard the disk-loaded one.
                            bitmap.recycle()
                            Timber.d("Discarded disk-loaded thumbnail for tab $tabId (in-memory version already present)")
                            existing
                        } else {
                            // No in-memory bitmap or it's recycled — use the disk-loaded one.
                    ...
                    val list = synchronized(pendingCallbacks) { pendingCallbacks.remove(tabId) }
                    list?.let {
                        synchronized(it) { for (cb in it) cb(finalBitmap) }
                    }
                }
            } catch (e: Exception) {
                ...
                mainHandler.post {
                    val list = synchronized(pendingCallbacks) { pendingCallbacks.remove(tabId) }
                    list?.let {
                        synchronized(it) { for (cb in it) cb(null) }
                    }
                }
            }
        }
    ...
    fun remove(tabId: Int) {
        ...
        val pending = synchronized(pendingCallbacks) { pendingCallbacks.remove(tabId) }
        if (pending != null) {
            mainHandler.post {
                synchronized(pending) { for (cb in pending) cb(null) }
            }
        }
    ...
    fun clear() {
        cache.evictAll()
        versionMap.clear()
        val allPending = synchronized(pendingCallbacks) {
            val snapshot = pendingCallbacks.values.toList()
            pendingCallbacks.clear()
            snapshot
        }
        if (allPending.isNotEmpty()) {
            mainHandler.post {
                for (list in allPending) {
                    synchronized(list) { for (cb in list) cb(null) }
                }
            }
        }
        ...
        diskExecutor.execute {
            try {
                ...
                for ((sessionName, filename) in sessionsSnapshot) {
                    ...
                    val bundle = com.xhub.browser.utils.FileUtils.readBundleFromStorage(application, filename)
                    if (bundle != null) {
                        try {
