...
import com.xhub.browser.extensions.createDefaultFavicon
...
open class TabModel (
    ...
    var webView : Bundle?,
    // Unique ID for the tab, persistent across sessions for thumbnail caching
    var tabId: Int = -1
)
{
    fun toBundle() : Bundle {
...
class TabModelFromBundle private constructor(
    ...
    companion object {
        ...
        operator fun invoke(bundle: Bundle): TabModelFromBundle {
            ...
            val tabId = bundle.safeGet(KEY_TAB_ID, -1) { getInt(it) }
            ...
            return TabModelFromBundle(bundle, url, title, desktopMode, darkMode, favicon, searchQuery, searchActive, webViewBundle, tabId)
        }
    }
}
